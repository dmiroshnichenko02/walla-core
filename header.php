...
<body <?php body_class(); ?>>
	...
	<div id="page" class="site">
		...
		<header id="masthead" class="site-header border border-[#F4F4F4]">
			<div class="h-[49px] bg-p2 w-full flex justify-center items-center gap-[73px] bg-no-repeat bg-top bg-cover" style='background-image: url("<?php echo $background_image['url'] ?>");'>
				<div class="flex gap-3 items-center">
					<img src="<?php echo $center_text_block['icon']['url'] ?>" alt="<?php echo $center_text_block['icon']['alt'] ?>">
					<div class='flex gap-[6px] items-center'>
						<h6 class='text-base leading-base text-black font-medium font-manrope'>
				...
				?>
					<a href="<?php echo esc_url($select_course['link']); ?>" class="px-4 py-[12px] text-[12px] leading-base text-white font-manrope font-medium bg-black cursor-pointer rounded-[5px]">
						<?php echo esc_html($select_course['text']); ?>
					</a>
				<?php
				elseif (!empty($select_course['text'])) :
				?>
					<div class="px-4 py-[8px] text-[12px] leading-base text-white font-manrope font-medium bg-black cursor-default rounded-[5px]">
						<?php echo esc_html($select_course['text']); ?>
					</div>
				<?php
				endif;
			...
			<div class="container mx-auto py-[15px] flex items-center justify-between">
				<a class='max-w-[150px] max-h-[50px] block mr-10' href="<?php echo esc_url(home_url('/')); ?>">
				...
				*/ ?>
				<div class="flex items-center">
					<div class="teach">
						<?php if (!empty($teach['url']) && !empty($teach['text'])) : ?>
							<a class='font-inter text-[14px] leading-[20px] font-medium text-grey-15' href="<?php echo esc_url($teach['url']); ?>"><?php echo esc_html($teach['text']); ?></a>
					...
					<div class="w-[1px] h-[10px] bg-[#D1D1D1] rounded-full mx-[6.5px]"></div>
					...
					*/ ?>
					...
					<div class="btns flex gap-2 items-center">
						<?php if ($is_user_logged_in): ?>
							...
							<a href="<?php echo esc_url(wp_logout_url(home_url())); ?>" class="px-6 py-3 rounded-[8px] border border-solid border-p2 text-black font-inter text-[14px] font-medium hover:bg-p2 transition">
								<?php _e('Logout', 'walla'); ?>
							</a>
						<?php else: ?>
							<!-- Пользователь не авторизован - показываем логин и регистрацию -->
							<a href="<?php echo esc_url($tutor_login_url); ?>" class="px-6 py-3 rounded-[8px] border border-solid border-p2 text-black font-inter text-[14px] font-medium hover:bg-p2 transition">
							<?php _e('Login', 'walla'); ?>
						</a>
							<a href="<?php echo esc_url($tutor_register_url); ?>" class="px-6 py-3 rounded-[8px] bg-p2 border border-solid border-p2 text-black font-inter text-[14px] font-medium hover:bg-transparent transition">
