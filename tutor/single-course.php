...
<section class="hero-course py-10 md:py-[22px] px-4">
	<div class="container m-auto">
		...
		<div class="hero-course-box bg-[#C1D8FF] rounded-[20px] md:rounded-[36px] p-4 md:py-8 md:px-11 flex flex-col gap-[20px] md:gap-8">
					...
					<div class="flex flex-col items-start gap-[10px]">
						...
						<div class="flex flex-col items-start gap-[10px] btn-section">
							<div class="w-full md:w-auto">
								...
								elseif ( $is_public && ! $is_purchasable ) {
			...
			if ($blocks_smb): ?>
				<div class="flex flex-wrap -mx-[6px] gap-y-2 md:gap-y-3">
					<?php
					$i = 0;
					$colors = [
						'bg-white',
					...
					?>
						<div class="w-full md:w-1/2 lg:w-1/4 px-[6px]">
						...
						<div class="w-full md:w-1/2 lg:w-1/4 px-[6px]">
							<div class="<?php echo $colors[$i % count($colors)]; ?> h-full rounded-[16px] p-[18px] flex flex-col min-h-unset md:min-h-[126px] gap-[14px] justify-center">
								...
								<p class="font-inter text-[rgba(0,0,0,0.7)] font-medium">
									...
									<?php if ($i % count($colors) === 3 && $timer_duration_seconds > 0): ?>
...
<section class="course-ai py-10 md:py-0 px-4">
	<div class="container m-auto">
		...
		<div class="course-ai-box bg-black rounded-[20px] md:rounded-[32px] py-5 px-4 md:py-10.5 md:px-10 flex flex-wrap lg:flex-nowrap gap-10 xl:gap-50 items-center">

			<div class="w-full md:min-w-[539px]">
				<img class="rounded-[12px] w-full min-h-[364px] object-cover" src="<?php echo get_field('ai_image_aib', 'option'); ?>" alt="">
			</div>
	...
	<div class="container m-auto">
		...
		<div class="content-course-box bg-[#FBFBFB] rounded-[20px] md:rounded-[32px] py-5 px-4 md:p-10">
			...
			<div class="flex flex-col md:flex-row items-left md:items-center justify-between gap-4 mb-8">
				...
				<?php endif; ?>
				<span class="expland-all cursor-pointer ont-inter font-semibold underline md:no-underline text-black md:text-[#5C77FF] capitalize text-[18px]">Expland All service</span>
			</div>
				...
				<?php if ($topics && $topics->have_posts()) : ?>
					<?php while ($topics->have_posts()) : $topics->the_post(); ?>
						...
						<div class="topic-item overflow-hidden first:rounded-tl-[24px] first:rounded-tr-[24px] last:rounded-bl-[24px] last:rounded-br-[24px]">
							<div class="topic-header <?php echo $is_topic_active ? 'active' : ''; ?> bg-[#F4F4F4] p-4 md:py-5 md:px-8.5 cursor-pointer group">
							...
							<div class="lesson-list <?php echo $is_topic_active ? 'active' : ''; ?> hidden overflow-hidden [.active]:block">
								<?php if ($topic_lessons && $topic_lessons->have_posts()) : ?>
									<?php while ($topic_lessons->have_posts()) : $topic_lessons->the_post(); ?>
										...
										<a href="<?php echo esc_url(get_permalink($current_lesson_id)); ?>" class="lesson-item <?php echo $is_current_lesson ? 'active' : ''; ?> flex items-center justify-between bg-white px-3 py-5 md:px-8.5">
...
<section class="course-certificate py-10 md:py-12 px-4">
	<div class="container m-auto">
		...
		<div class="course-certificate-box-wrap flex rounded-[32px] xl:p-10 xl:bg-[#F5F5F5]">
			<div class="course-certificate-box flex flex-col-reverse lg:flex-row rounded-[22px] md:rounded-[32px] p-4 md:p-10 gap-[54px] xl:gap-36.5 flex-wrap lg:flex-nowrap items-center bg-[#5C77FF] relative">
				<span class="block inset-0 absolute z-10 bg-cover" style="background-image: url(<?php echo get_stylesheet_directory_uri(); ?>/src/img/bg-cert.svg);"></span>
						...
						<?php if ($tags) { ?>
							...
							<div class="flex items-center gap-2">
								<?php foreach ($socials as $row) { ?>
									<a class="bg-[#E8E6FF] rounded-[8px] h-[27px] w-[27px] flex items-center justify-center" href="<?php echo $row['link']; ?>"><img class="h-[16px] w-[16px]" src="<?php echo $row['icon']; ?>" alt=""></a>
								<?php } ?>
							</div>
						<?php } ?>
					</div>
...
if ($immediately_blocks): ?>
	<section class="course-immediately py-10 md:py-12 px-4">
		<div class="container m-auto">
			...
			<div class="course-immediately-box flex flex-col rounded-[12px] md:rounded-[32px] py-5 px-4 md:py-10 md:px-7.5 gap-8 md:gap-14 bg-black">
			...
			<div class="feedback-slider-wrap overflow-x-hidden">
				<div class="feedback-slider">
					<?php foreach ($feedbacks as $row): ?>
						<div class="py-[33px] px-[26px] rounded-[24px] bg-white flex flex-col gap-8 min-h-[386px] justify-between">
