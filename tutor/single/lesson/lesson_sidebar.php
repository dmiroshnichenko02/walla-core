							...
							<a href="<?php echo $show_permalink ? esc_url( get_permalink( $quiz->ID ) ) : '#'; ?>" data-quiz-id="<?php echo esc_attr( $quiz->ID ); ?>">
								...
								<div class="tutor-d-flex tutor-ml-auto tutor-flex-shrink-0">
									...
									<?php if ( ! $lock_icon ) : ?>
										...
										$user_id = get_current_user_id();
										$quiz_id = get_the_ID();
										$is_completed_quiz = get_user_meta($user_id, '_tutor_completed_quiz_id_' . $quiz_id, true);
										$checked = $is_completed_quiz ? 'checked' : '';
										?>
										<input 
											type="checkbox" 
											class="tutor-quiz-toggle tutor-form-check-input tutor-form-check-circle" 
						...
						<div class="tutor-course-topic-item tutor-course-topic-item-lesson<?php echo esc_attr( get_the_ID() == $current_post->ID ? ' is-active' : '' ); ?>">
							<a href="<?php echo $show_permalink ? esc_url( get_the_permalink() ) : '#'; ?>" data-lesson-id="<?php the_ID(); ?>">
								...
								<div class="tutor-d-flex tutor-ml-auto tutor-flex-shrink-0">
									...
									if ( ! $lock_icon ) {
										$is_completed = tutor_utils()->is_completed_lesson( get_the_ID() );
										$checked = $is_completed ? 'checked' : '';
									
										$markup = '<input 
											type="checkbox" 
											class="tutor-lesson-toggle tutor-form-check-input tutor-form-check-circle" 
											data-lesson-id="' . get_the_ID() . '" 
											' . $checked . ' 
										/>';
