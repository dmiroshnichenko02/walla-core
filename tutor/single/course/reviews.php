...
if ( ! $course_id ) {
    $maybe_course_id = tutor_utils()->get_course_id_by_subcontent( get_the_ID() );
    $course_id       = $maybe_course_id ? $maybe_course_id : get_the_ID();
}
...
<div class="tutor-pagination-wrapper-replaceable">
	...
	<?php if ( ! is_array( $reviews ) || ! count( $reviews ) ) : ?>
		<?php if ( $my_rating && isset( $my_rating->comment_approved ) && 'hold' === $my_rating->comment_approved ) : ?>
			<div class="tutor-card tutor-p-24 tutor-mb-24">
				<div class="tutor-fs-6 tutor-color-secondary tutor-mb-8"><?php esc_html_e( 'Your review is awaiting moderation', 'tutor' ); ?></div>
				<div class="tutor-d-flex tutor-align-center tutor-mb-12">
					<div class="tutor-ratings tutor-ratings-lg">
						<?php tutor_utils()->star_rating_generator_v2( tutor_utils()->get_rating_value( $my_rating->rating ?? 0 ), null, false ); ?>
					</div>
				</div>
				<div class="tutor-color-black tutor-mt-8">
					<?php echo esc_html( $my_rating->comment_content ?? '' ); ?>
				</div>
			</div>
		<?php else : ?>
